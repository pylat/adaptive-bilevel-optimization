...
function run_Linverse_sim(; seed = 3, m = 400, n = 1000, pf = 5, tol = 1e-5, maxit = 10_000)
    ...
    @info "solver with bilevel problem with projected gradient linesearch method"


    @info "solver with Iterative3_D method"
    sigma0 = 1

    sol, numit, record_Iterative3_D = Iterative3_D(
        zeros(n),
        f1 = f1_BiGSAM,
        f2 = Counting(f2),
        sigma = sigma0,
    ...
    to_plot = Dict(
        ...
        "Iterative3D" => concat_dicts(record_Iterative3_D |> subsample(100)),
    )
